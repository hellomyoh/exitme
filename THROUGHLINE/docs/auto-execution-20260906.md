...
08:30~09:10  워커 poll_expected_open (app/preopen_watch.py, 매 분) → KODEX·TIGER 200 예상체결가(08:59 까지)/확정 시가(09:00~) → Redis → 주문표에 '예상 시가 … 갭 기준 위/이하' 표시(표시 전용)
...
화면  주문표 위 상태 한 줄(수동/대기/실행 중/완료/취소됨/정지/경고 + 사유) · 버튼은 [이번 실행일 무인 취소 → 수동]·[되돌리기]·[다시 켜기]·[재시도](장중, 실패·생략 줄만 — 2026-09-09)만 · 표의 '무인' 열은 줄별 최종 상태
```
...
| 웹 | `portfolio/page.tsx`, `settings/page.tsx` | 상태 배너 + 취소/되돌리기/다시 켜기/재시도(2026-09-09) · '무인' 열(읽기 전용) · 제목의 동결/실시간 표기 / 계좌별 플래그 2개 + 상한 입력, 켬·끔 확인창(끄면 즉시 취소 안내) |
...
5-1. 09:01 발주가 KIS 오류로 **실패**했거나 생략된 줄이 있으면 '무인' 열 헤더의 **[재시도]**(장중 09:00~15:20)로 그 줄만 같은 절차로 다시 넣는다(`POST …/auto-exec/retry`, 2026-09-09). 대조 불일치로 정지된 날도 원장을 맞추고 "다시 켜기" 뒤 재시도하면 그날 발주를 이어갈 수 있다. 갭 생략·발주된 줄은 대상 외.
