...
- 평일 배치 시각(KST): 08:57 사전 갭 취소 → 09:01 무인 실행 → 15:45/17:10 장 마감 동기화(체결·주문 상태·예수금 대조) → 16:05 일봉 → 16:40 스냅샷·일일 현황 알림 → 16:45 자동 승인. 모두 worker/scheduler 컨테이너가 돌리므로 배포 뒤 두 컨테이너가 떠 있는지 확인한다(`docker compose ps`).
