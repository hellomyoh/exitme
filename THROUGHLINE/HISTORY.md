...
## [2026-09-12] research | Moreira·Muir(2017) 분석 + 세 제안 검증 (사용자 지시)

- 논문(JF 2017, 「Volatility-Managed Portfolios」) 원문을 읽고 구성(c/RV², 22일 실현분산, 월 리밸런싱)·결과(시장 α 4.86%, appraisal 0.34, 샤프 0.42→0.51, MV 효용 +65%)·기전(α ∝ −cov(μ/σ², σ²), 침체기 β 0.83→0.32, VAR: 충격 즉시 −50%·12개월)·레버리지/거래비용 표(1/RV 변형 회전 절반·샤프 동일, 손익분기 56~161bp)·한계(변동성의 변동성에 의존, 1956–85 약함)를 정리하고 RAVG v2.5 와 대응시켰다 — RAVG 의 1/σ 노출·상한은 논문이 권하는 저회전 변형 계열, 추세 레짐 오버레이와 전술 눌림목 트랙은 논문에 대응물 없음.
- 세 제안 검증(1억, 2017~2026 전 구간 + 89개 1년 창 짝 비교, 비용 후):
  - **P1 상승장 진입 가속 — 기각**: 세 변형 CAGR −0.12~−0.18%p, MDD −0.9~−1.6%p, 89창 중 19~20승. 목표 90% 도달 15.6 → 13.6일뿐. 상승장 K200 목표(70%)는 중립장에서 이미 채워져 미달이 작고, 실제 미달은 레버리지 레그에 있다.
  - **P2 전략:전술 비율 — 채택 후보(85:15 권고)**: 전술 트랙은 8.6년 비용 후 **−88만원**(전략 +6,340만) — 왕복이 과세·슬리피지에 갉힌다. 85:15 CAGR 17.79%(+1.14%p)·샤프 1.05·MDD −22.7%(−0.8%p); 100:0 CAGR 18.32%(+1.67%p)·MDD −23.6%(−1.7%p)·체결 −21%. 짝 비교 평균 +1.80/+2.73%p 이나 중앙 +0.17/+0.29%p 로 소수 상승 국면에 집중, MDD 악화 창 73/76 of 89. 정본 §7 비율 변경이라 ADR·사용자 결정 필요.
  - **P3 리밸런싱 밴드 — 현행 5% 유지**: 7% 는 체결 −3%(축소 매도 99→92)뿐인데 CAGR −0.33%p·89창 중 4승; 3% 는 MDD −21.1%(+0.8%p)지만 체결 +13%·최악 창 −3.74%p. 과노출(>5%p) 일 비율은 0.0~0.4% 로 어느 밴드든 작다 — 레짐 전환 축소가 밴드를 우회. 회전의 대부분은 그리드·익절 왕복이라 밴드로 줄일 수 없다.
- 방법: P1 은 `backtest.plan` 래퍼(운영 코드 불변), P2·P3 는 기존 파라미터. 문서 `docs/vol-managed-review-and-three-proposals-20260912.md`.
- 테스트 결과: 문서만 — 코드 변경 없음, 직전 309 passed 유지.
- Git commit: docs: review Moreira-Muir 2017 and test three proposals (bull-entry boost, leverage split, rebalance band)

## [2026-09-12] strategy | 레버리지 전략:전술 70:30 → 85:15 (사용자 지시)

- 지시: "85:15 이거 적용해서 현재와 비교 분석해줘". 정본 §10 튜닝표 `레버리지 트랙 비율`(○ 튜닝 허용) 범위 내 값 조정이라 `grid_max` 때와 같이 ADR 없이 feature 명세가 확정값을 가진다(SOURCES 불변).
- 변경: `Params.lev_strategic_ratio` 0.7 → **0.85**. 총 레버리지 목표(w_LEV)·K200 목표·레짐 규칙은 불변 — 레버리지 예산 안의 배분만 바뀐다. 전술 트랙(눌림목 진입·EMA 회귀 이탈) 30% → 15%.
- 전 구간(1억, 2018-01-25~2026-09-10, 비용 후): 누적 261.68% → **292.24%**(+30.56%p), CAGR 16.65% → **17.79%**(+1.14%p), MDD −21.86% → **−22.67%**(−0.80%p), 샤프 1.027 → 1.049, 최종 자산 +3,056만원. 레버리지 **보유일은 40.04% 로 동일**하고 평균 비중이 8.04% → 9.63% — 더 오래가 아니라 같은 날 더 많이 든 결과다.
- 손익 원천: `lev_strat` +2,044만, `core` +812만(equity 증가의 2차 효과), `grid` +179만. 전술 트랙은 두 비율 모두 순손실(−88만 → −53만).
- 연도별: 9년 중 2020(+3.4%p)·2025(+6.3%p)·2026(+3.1%p) 세 해가 +12.8%p 를 만들고 나머지 6년 합계 +0.1%p. 하락·횡보 연도는 레버리지가 0 이라 차이 없음. 레짐별로도 BULL 만 변화(실효노출 97.9% → 103.8%).
- 낙폭: 구간 자체는 동일(레짐·그리드 불변), 깊이만 0.4~0.9%p 깊어지고 **회복은 빨라진다**(코로나 2020-11-11 → 08-12, 2025년 07-03 → 06-24).
- 시작일 89개 1년 창 짝 비교: 평균 +1.80%p, **중앙 +0.17%p**, 최악 −0.80%p, 최선 +19.01%p, 이김 49/89. **MDD 는 73창 악화·0창 개선**(평균 −0.47%p). 이득은 2025년 시작 창들에 몰리고 비용은 거의 모든 창에 고르게 발생한다.
- 민감도(무릎점 없음): 0.75/0.80/0.85/0.90 의 창 평균 +0.98/+1.53/+1.80/+1.91%p, MDD 평균차 −0.13/−0.29/−0.47/−0.62%p, 악화 창 28/55/73/75 — 낙폭 1%p 당 수익 효율이 7.5 → 3.1 로 단조 감소하는 **위험 선호 다이얼**이다. 0.80 → 0.85 증분만 보면 중앙값 −0.03%p·이김 37/89 로 전 구간에서 가장 불리한 한 칸이라, **0.80 을 대안으로 문서에 남겼다**(사용자 결정 대기).
- 테스트 결과: **323 passed**(코드 변경은 기본값 1줄이라 신규 테스트 없음 — 기존 레버리지 테스트 9건이 새 비율에서 그대로 통과), `tsc --noEmit` 무오류. 라이브 설정 API 로 기본값 0.85 반영 확인(사용자 지정 override 0명).
- 한계: 한 시장·8.6년·상승 진입 12회·겹친 창. 이득의 절반 이상이 2025~2026 상승 국면 하나에서 나온다. 지금 고정했으므로 이후 구간이 표본 외 검증이 된다.
- 문서: `docs/lev-split-8515-20260912.md` 신규, feature-strategy-engine §5.5·§15, docs/strategy-guide.md, 웹 RAVG 가이드.
- Git commit: strategy: shift the leverage split from 70:30 to 85:15

## [2026-09-12] research | emax_bull 인하 검증 — 낙폭을 줄이려면 어디를 손대야 하나 (사용자 지시 "검증하세요")

- 배경: 85:15 적용 후 "낙폭이 부담이면 손댈 곳은 트랙 비율이 아니라 노출 목표(emax_bull)"라고 적은 판단을 실제로 쟀다. 비교축 A 종전(0.70/1.30) · B 현행(0.85/1.30) · C(0.85/emax 인하 1.25~1.00).
- 전 구간(1억, 2018-01~2026-09, 비용 후): emax 1.30 → 1.00 에서 CAGR 17.79% → 14.48%, MDD −22.67% → −18.58% 로 **단조 감소**하고 **상승장 목표 충족률은 91~92% 로 유지**된다 — 목표가 내려간 것이지 이행이 나빠진 게 아니다. 전술 비중(충족률 86%)과 달리 `emax_bull` 은 원하는 노출을 그대로 주는 손잡이임이 확인됐다.
- 핵심 질문(같은 낙폭에서 수익): 종전 낙폭 −21.86% 에 대응하는 값은 **0.85 / emax 1.20**(−21.62%)이고 CAGR 16.51% — 종전 16.65% 와 **사실상 동일**(최종 자산 −355만원). **성과 개선은 없고 얻는 것은 통제 방식**이다: "눌림목이 안 오면 저절로 덜 산다"(우연·비문서화·상승장 한정) → "목표를 1.20 으로 정한다"(명시·일관).
- 89개 1년 창 짝 비교(기준 A): B 는 평균 +1.80%p·MDD 악화 73창/개선 0. C 1.20 은 평균 −0.14%p·MDD 악화 10창/개선 9창으로 **A 와 구별되지 않는다**. C 1.15·1.10 은 최악 창이 −10.70%p·−26.76%p 로 기회비용 꼬리가 폭발 — 2025~2026 상승 국면을 놓치는 비용이다.
- 창별 낙폭 분포: "1년에 −20% 넘게 빠지는 창"은 A·B·C1.20·C1.15 모두 3개이고, 실제로 0 이 되는 건 **emax 1.10**(창 평균 −2.07%p, 최악 창 −26.76%p). 낙폭 상한 −20% 는 이 표본에서 매우 비싼 선택.
- 판정: **1.15 미만 비권장.** 결정은 "−22.7% 를 감당하는가" 하나 — 감당하면 현행(0.85/1.30) 유지, 아니면 `emax_bull` 1.20. 어느 쪽이든 전술 비중 0.85 는 유지한다(손실 규칙 축소 + 목표 이행률 개선이라는 별개 사안).
- 테스트 결과: 코드 변경 없음(파라미터 미적용, 문서만) — 직전 323 passed 유지. 측정은 `scratchpad/emax.py`(전 구간 8구성 + 89창 5구성).
- 문서: `docs/lev-split-8515-20260912.md` §6 추가.
- Git commit: docs: verify emax_bull as the drawdown lever

## [2026-09-12] fix | '오늘 손익'이 주말·휴장·장 시작 전에 0 으로 보이던 문제 (사용자 지적)

- 지적: "주말이면 금요일 데이터 기준으로 표기되어야 할 것 같다" — 토요일 대시보드의 오늘 손익이 0원(0.00%)이었다.
- 원인: 현재가·비교 기준가가 **둘 다 '오늘 이전 마지막 종가'** 라 새 봉이 없는 날 같은 값이 됐다. 총자산 카드는 오늘 스냅샷(마지막 종가로 평가)과 어제 스냅샷이 같아져 0. 주말뿐 아니라 **공휴일·평일 장 시작 전·일봉 적재 지연**에도 같은 일이 벌어졌다(개발 DB 확인: 캘린더는 09-11 개장인데 봉은 09-10 까지만 있어 금요일에도 0).
- 규칙 확정: **기준은 지금 쓰는 가격 바로 직전 값**. 실시간 시세가 있으면 현재가 vs **오늘 이전** 마지막 종가(애프터마켓에 오늘 정규장 종가가 이미 적재돼 있어도 하루 전체 변동을 보여야 함 — 회귀 테스트가 이 경우를 잡았다), 없으면 마지막 종가 vs 그 직전 종가 → 마지막 거래일의 변동. 캘린더가 아니라 **데이터(봉)** 로 판정해 적재 지연도 자동으로 드러난다.
- 작업 내용: `dashboard.last_two_closes`·`day_change_basis`·`latest_bar_day` 신설, 시장 카드·계좌 행·총자산 카드(기준일 스냅샷 비교)·매매일지 행(`mjournal._last_two_close_map`)에 적용. 기준일이 오늘이 아닐 때만 `*_asof` 를 응답에 싣고 화면은 `오늘` 대신 `9/11` + `종가 기준`으로 표기(카드·표 셀 모두). 총자산 기준일 산출에 매매일지 보유 종목도 포함 — 일지에만 보유한 사용자도 기준일이 잡힌다.
- 테스트 결과: **331 passed**(신규 7 — `tests/test_day_change_asof.py`: 마지막 두 종가 선택·장중 예외·봉 1개 종목 제외·주말 시장카드/계좌행·총자산 스냅샷 비교·오늘 봉 있으면 종전 동작·매매일지 행). `tsc --noEmit` 무오류.
...
## [2026-09-13] research | 레짐 판정 분석서 검증 — 세 제안의 도입 판정 (사용자 지시)

- 분석서 수치 재계산: 2% 완충 전환 41/83회·짧은 상승 구간 5/12개·중앙 20/7일·2026-09-09 사례(+20.06%/−7.64%, 중립) 전부 일치. 직행은 "상승→하락 2회"가 아니라 **양방향 5회**(그중 4회 미세 교차). 완충 제거 시 누적 292.2% → 271.8%, MDD −22.7% → −26.6% — **완충은 성과로도 정당화**.
- 제안 1(중립 세분화 조정/회복/경계): 두 중립은 다른 상태 — 조정 53일(전략 일변동 1.20%, 20일 적중 42%), 회복 94일(0.53%, 62%). 다음 레짐 예측력은 없음. **표시·진단으로 채택, 배분 차등은 보류**(표본 작음, 차이는 이미 경로 의존으로 반영).
- 제안 2(미세 직행 하루 확인): 전 구간 누적 +18.8%p·샤프 1.049→1.084·MDD 불변으로 보이지만, 사건별로 보면 **현행 경로의 5회 직행은 하나도 되돌아가지 않았고** 확인 규칙은 진입·유지의 비대칭(진입이 더 엄격) 때문에 반대 레짐에 더 머무는 **진입 강화 규칙**으로 작동. 89창 짝 비교 이김 23·짐 22·동일 44, MDD 악화 21·개선 13, 이득 상위 5창이 전부 2024-08~12 시작(2025-02 사건 하나), 임계 1% 에서 부호 반전. **채택하지 않음** — 하방 창 22개가 `grid_max` 채택 때(0개)와 다르다.
- 제안 3(추세·위험 분리 표시): 레버리지 차단 사유(σ20>35%·E≤1·하락장)가 화면에 없음. 서버 계산 불변, 라벨만 — **표시로 채택**.
- 테스트 결과: 코드 변경 없음(운영 코드·주문 규칙 불변). 측정 `scratchpad/regime_review.py`·`regime_p2_detail.py`(P2 는 `planner.next_regime` 래퍼).
- 문서: `docs/regime-review-verification-20260913.md`.
- Git commit: docs: verify the regime review - adopt the neutral split and risk label as display, reject one-day confirmation

## [2026-09-13] research | 레짐별 매매 타이밍 검토 — 하락장 저가 매수·상승장 익절·진입 가속 (사용자 지시)

- 질문: "상승·하락장에서 매수/매도 타이밍을 빠르게 할 필요는 없나 — 하락장 촘촘한 저가 매수, 상승장 익절 매도". 20개 변형을 `backtest.plan` 래퍼로 시뮬레이션(운영 코드·파라미터 기본값 불변), 2018-01-26~2026-09-10 신호일 2,113개·자본 1억·비용 후.
- 전제 정정: **매도는 이미 빠르다** — 하락장 축소는 밴드 무시 익일 시가 시장가 50건, 상승장에도 매도 76건(축소 24·레버리지 전략 22·전술 이탈 19·강제청산 11). 방아쇠가 가격이 아니라 위험(E)일 뿐. 실현손익의 98%가 상승장 보유분(core +19,329만원·레버리지 전략 +9,404만원).
- 상승장 익절: 폭 1×/2×/3×/5×/8×Grid → 누적 92.6/108.8/120.6/154.5/200.5% vs 현행 292.2% — **팔수록 잃고 단조**(m→∞ 가 현행). 상승장 노출 충족률 92%→64~86%. 엔진 절제 플래그 `f1_no_tp_in_bull=False` 대조 84.0%로 같은 방향. 과열 분할매도(EMA20+k×ATR)도 220.8~259.5%·샤프 전부 현행 미만. 89창 짝 비교(1×) 평균 −12.20%p·짐 49·이김 33, 단 MDD 는 76창 개선(샤프 1.049→0.670 이라 비효율). **미채택**.
- 하락장 저가 매수: 여력은 실재 — 하락 875일 중 **595일**이 목표(E=0.20) 대비 1%p 초과 미달, 평균 미달 6.41%. 그러나 그리드 재가동 4변형(0.5×/1.0×/2.0×/5단) 모두 누적 287.0~288.6%, 해당 매수 로트의 왕복 손익 **−145~−223만원**·승률 45~51%, 89창 MDD 악화 21·개선 1. 투매일(MA20−k×ATR) 매수는 291.6~292.1%로 무차별(MDD 악화 17·개선 11). **미채택**.
- 상승장 진입 가속: 얕은 사다리·종가 보충 15/30% → 누적 297.8~302.3%·충족률 92→93~95% 지만 89창 이김 21~27·짐 54~61, MDD 악화 32~47창(이득이 소수 창에 집중). **보류** — 재검토 조건은 85:15 적용 뒤 실계좌 충족률 저조 관측.
- 테스트 결과: 코드 변경 없음(운영 코드·주문 규칙·기본값 불변). 측정 `scratchpad/regime_timing_study.py`·`regime_timing_detail.py`·`regime_timing_alt.py`.
- 문서: `docs/regime-timing-study-20260913.md`.
- Git commit: docs: test regime-conditional entry and exit timing - keep the current asymmetry

## [2026-09-13] fix | 2026-09-11 09:01 발주 실패(EGW00215) 원인 분석·조치 (사용자 지시)

- 사건: `M-한투-ETF` 그리드 1·2·3차 매수 3줄이 "잔고 조회 실패로 발주하지 않았습니다 — KIS error EGW00215 원장에서 허용 가능한 초당 거래건수를 초과하였습니다"로 생략. 원인 3겹 — ① `_get` 이 EGW00201 외의 코드를 즉시 오류로 올려 **EGW00215 는 재시도 대상이 아니었다** ② 잔고 조회 실패는 그 계좌의 남은 줄 전부를 생략한다(원장 대조 안전장치) ③ 09:15 감시는 '실행 기록 없음'만 보므로 이미 돈 포트는 건너뛰어 **아무도 다시 내지 않았다**.
- 유량 코드는 두 층이다: EGW00201 = 게이트웨이(앱키) 초당 한도(2026-09-09 사고로 Redis 공용 카운터 10/초 적용 중), EGW00215 = **원장(계좌) 초당 한도** — 앱키 카운터로는 막히지 않는다. 09:01 배치는 계좌마다 시가 1 + 잔고 1 + 줄마다 매수가능 1 + 주문 1 을 0.15초 간격으로 연속 호출한다.
- 조치: ① EGW00215 를 유량 코드로 인정해 GET 4회·주문 POST 3회 백오프 재시도(접수 전 거절이라 중복 위험 없음) ② 원장(계좌) 단위 초당 카운터 추가 — 실전 2건/초·모의 1건/초, Redis 없으면 인스턴스 0.4초 간격. 잔고·매수가능·체결·미체결·예약 조회와 모든 주문 POST 가 대상(시세 제외) ③ 200+`rt_cd≠0` 형태의 유량 초과도 재시도 ④ **09:15 감시가 일시적 생략만 재발주** — 생략 사유가 조회 실패(`skip_cause="api"`)인 줄과 유량 초과로 실패한 줄만. 갭 취소·플래그 꺼짐·잔고 부족·상한·대조 불일치는 대상 외 ⑤ EGW00201·EGW00215 사용자 안내 문구.
- 테스트 결과: `pytest -q tests/` → **340 passed** (신규 5 — EGW00215 GET 재시도·200 본문 형태 재시도·타 코드 즉시 실패·주문 POST 재시도·원장 카운터 계좌별 분리·09:15 감시가 일시적 생략만 재발주). 기존 stub 3곳에 `ledger` 인자 추가.
- 남는 위험: 09:15 재발주는 지정가 줄은 같은 가격, **시장가 줄(축소·레버리지)은 09:15 시세**로 나간다(사용자가 누르던 '재시도'와 같은 성질). EGW00215 의 정확한 임계는 KIS 미공개라 2건/초는 보수적 가정이며 실계좌 재현은 하지 않았다.
...
## [2026-09-13] research | 성과 검증 방법론 — 워크포워드·블록 부트스트랩·미사용 구간 (사용자 지시 "모두 진행")

- 문제: 지금까지의 채택 근거(`grid_max` 2.5%, 85:15, 레짐 제안들, 레짐별 타이밍)가 **모두 같은 KR 8.6년 한 경로 + 겹치는 89창**이었다. 세 가지 독립적 방법으로 그 근거의 단단함을 쟀다. 운영 코드·파라미터 불변.
- **블록 부트스트랩**(21일 블록·2,000회, 실현 일수익률 재표집): CAGR 90% 구간 **4.53~32.20%**(실측 17.79%), MDD **−41.63~−17.21%**(−22.67%), 샤프 **0.363~1.692**(1.049). 실측은 중앙값 근처 — 운 좋은 경로가 아니다. 그러나 **누적 수 %p 차이는 신호가 아니다**: 진입 가속 보류(+10%p)가 사후 정당화되고, 상승장 익절(−200%p)은 밴드 밖이라 진짜 열위다.
- **워크포워드**(24조합을 그 해 이전 샤프로 선택, 보유·현금을 이어받아 그 해 평가): 선택 288.3% vs 기본 282.8% — 8년 **+5.5%p = 연 +0.19%p**, 이긴 해 5/8 이지만 진 해의 손실이 더 크고(−5.0·−5.0) MDD 도 −24.15% vs −24.05% 로 근소 악화. **연 단위 재튜닝 미채택.** 정보가 있는 부분은 방향 — 선택기는 8년 내내 `grid_max` 2.0%, 2022년부터 전략 트랙 100% 를 골랐다(우리 채택과 같은 쪽, 더 멀리). 이득이 노이즈 안이라 추가 이동은 안 한다.
- **미사용 구간**(미국 QQQ/QLD 2008-2017 — KR 튜닝이 본 적 없는 시장·10년, KR 기본값 그대로·미국 비용): 누적 186.6% · CAGR 12.01% · MDD −26.23% · 샤프 0.835 로 동시기 구간(13.39% / −24.61% / 0.804)과 거의 같다 — 과최적화 징후 없음. 재확인: 85:15 는 70:30 대비 누적 +7.7%p·MDD +1.0%p·샤프 −0.016 으로 **KR 과 부호가 같고**, `grid_max` 2.5% 는 그 구간에서 상한이 한 번도 물리지 않아 4.0% 와 **완전히 동일** — 확인도 반박도 불가.
- 채택 기준에 한 줄 추가: 전 구간 누적 차이만으로 채택하지 않는다. 기존 기준(창별 승패·MDD 악화 개수·상한이 물리는 날 수·충족률·사건별 경로)이 노이즈에 강한 이유가 수치화됐다.
- 테스트 결과: 코드 변경 없음. 측정 `scratchpad/validation_study.py`·`validation_wf2.py`. 한계: 부트스트랩은 지표의 표본오차이지 전략 행동의 재현이 아니고(블록 길이 민감), 워크포워드는 연 단위로 끊어 로트 종류·익절가가 초기화되며(기본 트랙 282.8% ≠ 단일 실행 292.2%), 미국은 비용 프로파일만 교체(세금·분배금·환율 미반영, 실제 미국 정본은 TF/LTM).
- 문서: `docs/validation-methodology-20260913.md`.
- Git commit: docs: measure the noise floor - bootstrap bands, walk-forward and an untouched market

## [2026-09-13] fix | 사전 잔고 조회(08:45) + 잔고 조회 실패 시 발주 계속 (사용자 지시 2건)

- 지시 1 "주문 30분 전에 잔고를 확인하고 주문": `auto_exec_prefetch_balance` Celery 태스크 추가(평일 **08:45**) — 무인이 켜진 국내 포트마다 잔고를 한 번 받아 Redis(`autoexec:balance:{pid}`, TTL 1h)에 담고 09:01 이 그대로 쓴다. **08:45 인 이유**: 장전 시간외 종가매매가 08:30~08:40 이라 그 뒤로는 개장까지 보유·예수금이 바뀔 수 없다. 캐시는 **09:05 까지만** 유효 — 개장 뒤에는 체결로 잔고가 변하므로 09:15 감시·장중 재시도는 새로 조회한다. 사전 조회가 실패해도 09:01 이 직접 조회한다.
- 효과: 09:01 의 계좌 원장 호출이 `시가1 + 잔고1 + 줄마다(매수가능1 + 주문1)` → `시가1 + 줄마다(매수가능1 + 주문1)`. 한 건이지만 **그 한 건이 2026-09-11 에 그날 전부를 막은 호출**이었고, 08:45 로 옮기면 실패해도 09:01 까지 15분 여유가 생긴다.
- 지시 2 "잔고 조회 실패로 주문이 안 들어가는 건 비효율": 조회 실패는 더 이상 줄을 막지 않는다. 근거 — 현금·보유가 모자라면 **증권사가 거절**하므로 조회 실패가 곧 "주문하면 안 되는 상태"는 아니다. 실패 시 매도 상한은 **원장 보유**, 매수가능조회까지 실패할 때의 폴백 예산은 **원장 현금**, 줄마다 `잔고 확인 불가 — 원장 기준 발주` 사유를 남긴다. **조회 성공 + 불일치**(원장 ≠ 계좌)는 종전대로 전량 생략 + 포트 정지 — 완화한 것은 '실패'뿐이다.
- 잃는 것은 그날의 사전 대조 한 번. 보완: 조회되는 날의 불일치 판정 유지 · 수량이 틀리면 증권사 거절로 `failed` 기록(재시도 대상) · 장 마감 동기화가 체결을 원장에 맞춘다.
...
## [2026-09-13] research | 레짐 논문 검토서 검증 — JM/HMM 레짐·비용-기대손익 목표비중·체결 조건부 가격 (사용자 지시)

- 제출된 검토서(Ang·Bekaert, Shu·Yu·Mulvey, Gârleanu·Pedersen, Cont·Kukanov)의 세 연구안을 실제로 시뮬레이션했다. 운영 코드·파라미터 불변.
- **A. JM/HMM 레짐** (검토서 목적함수 그대로, 롤링 학습창 500/750/1000일, 21일마다 재학습, **전방 재귀·필터 확률**로만 판정, 현행 `next_regime` 자리에 주입 — 나머지 규칙 현행): 33개 변형 중 누적·MDD·샤프 셋 다 현행을 넘은 것 **0개**. 현행 303.6%/−22.67%/1.156(500일 창) vs 최상 JM3 λ=50 258.9%/−29.65%/1.110, HMM 157.6%/−38.20%/0.851. 2상태는 중립이 없어 MDD −30~−52%(1000일 창 −22~−33%), 3상태는 중립을 33~68% 로 잡아 누적이 뒤진다(JM 상태 ≠ MA 레짐이라 고정 Emax 에 맞지 않음). 최상 후보 1년 창 짝 비교 이김 35:짐 37 / 19:29. **미채택**.
- **B-수량. 비용·기대손익 목표비중** (μ̂ = 현행 레짐별 향후 H일 수익의 완전 관측 표본 평균·축소, Σ̂ 250일×H 대각 25% 축소, c 편도 0.115%, 격자 최적화, 현행 레짐 상한 유지 — **같은 시장가 실행기**로 현행 E 목표와 비교, 2019-01~2026-09): 현행 295.0%/−29.87%/**0.937** vs MV 14~290%/샤프 **0.56~0.66**(γ 6종·축소 n0 {250,50,0}·H {20,60}). 수익이 붙는 유일한 설정(H=60, 289.8%)은 MDD −42.9%. 78창 짝 비교 전 변형 짐 45~49:이김 29~33. 원인: 레짐별 μ̂(20일)가 상승 −4.25~+2.81%, 중립·하락은 **중앙값이 음수** — 기대수익이 추정되지 않는다. 비용항의 유일한 효과(회전율 절반)는 현행 밴드·그리드에 이미 있다. **미채택**.
- **B-가격. 체결 조건부 기대손익 가격**: 검토서 지적대로 일봉으로 체결확률 인증 불가 — **검증 안 함**. 진단만: 현행 그리드 매수 체결의 20일 선행수익은 전 그룹 양(+)(0.6~10.7%), 가장 약한 곳은 상승장 1차 **갭 체결**(+0.60%, 25건, 64%).
- 공통 결론: 세 안은 "기대수익을 추정해 쓴다"로 가고 현행은 추정하지 않는다(변동성 기반 E + 추세 위치 레짐). 2,374봉에서 국면별 기대수익은 부호도 안정적이지 않다 — 2026-09-12 Moreira·Muir 검토와 같은 결론. 검토서의 평가 규율(시간순 분리·탐색 횟수 기록·필터 확률)은 그대로 지켰고 이미 관행.
- 테스트 결과: 코드 변경 없음. 측정 `scratchpad/jm_study.py`·`jm_windows.py`·`mv_cost_study.py`·`mv_sens.py`.
- 문서: `docs/regime-paper-review-20260913.md`.
- Git commit: docs: test the paper-based regime and allocation proposals - none beats the current rule

## [2026-09-13] research | 초기 진입 검토서 검증 — 공동 예산·수량 배분(A)·체결 후 순이익 가격(B)·진입 비율(C) (사용자 지시)

- 검토서의 코드 사실 관계(초기 진입 10일·15%·종가, 그리드 식, 85:15, **그리드 단은 현금 부족 시 생략·레버리지는 축소**라는 비대칭, 첫 10일 = 계좌 시작 기준, 백테스트 시가 ≠ 실전 09:01) 전부 코드와 일치 확인. 실행 시점 차이는 `minute-fill-study-20260901.md` 가 1년 분봉 대조로 일봉 엔진 유지 판정.
- 진단(현행, 비하락장 계획일 1,229개): 현금 부족으로 통째로 생략된 그리드 단 **82**(단·일 3,687개의 2.2%), 그리드 예약 때문에 축소된 레버리지 전략 매수 **7**. 검토서 지적은 사실이나 드물다.
- **A1 생략→축소(수수료 포함 예산)**: 8.6년 47단 살아남, 누적 292.2→292.3%, 89창 이김 10·짐 1·동일 78, 최대 +0.09%p, MDD 변화 0 → **채택 가능 — 정합성 개선, 수익 무영향**(검토서 자신의 근거와 일치). **A3 레버리지 우선**: 4일 변경 전부 손해(누적 −5.7%p, 4창 짐·0창 이김, 최악 −4.20%p) — 더 싼 K200 단이 체결 시점에 탈락하는 기전 → **미채택**. **A13 공동 배분(레버 우선→그리드)**: 누적 −1.8%p, 이김 40·짐 5 이지만 평균 −0.02%p → **미채택**. 현행의 "그리드 먼저"가 옳다.
- **B**: 모형 없이 비교 가능한 후보(전일 종가 지정가·얕은 사다리·종가 보충·시장가 초기 진입)는 기존 연구에서 전부 열위. 고르는 부분(체결 조건부 기대수익)은 어제 μ̂ 불안정성과 같은 추정 문제 + 표본 234건. 체결확률은 호가 자료 없어 **검증 불가**. **C**: 상수 η 는 진입 가속 검토로 이미 보류(창 54~61개 짐), 예측 기반 η 는 B 와 같은 문제 → **보류**. "위험 축소와 진입 속도 분리"는 현행이 이미 그렇다(`planner.py:202`).
- 받아들이는 것: 검토서의 검증 설계(기준/A/B/C 분리·09:01 이후 경로·중첩 창 독립 표본 아님·스트레스). 결합 최적화의 병목은 방향이 아니라 **입력(기대수익·체결확률)의 부재**.
- 테스트 결과: 코드 변경 없음. 측정 `scratchpad/entry_alloc_study.py`.
- 문서: `docs/entry-allocation-review-20260913.md`.
- Git commit: docs: test the entry-allocation proposals - clip instead of skip is harmless, leverage-first loses

## [2026-09-13] fix | 진입 배분 연구 정정 (r2) — 반론 검증, 결함 재현, 판정 수정 (사용자 지시)

- 제출된 반론서를 검증했다. **핵심 지적 두 가지가 옳았다.** ① 1차 실험 래퍼의 `lev_plan()` 이 `lev_liq` 매도 유무만 보고 **σ20 > 35% 게이트를 확인하지 않아**, 보유가 없는 날 금지된 레버리지 매수를 만들었다 ② 추가한 그리드 단이 기존 레버리지 주문의 현금 점유를 빼지 않아 **공동 예산이 아니었다**(반론서 실측 예산 위반 A1 12일·A13 29일).
- 결함 직접 재현(`scripts/verify_entry_alloc_r1_defect.py`): 1차 A3 에서 바뀐 4일은 **2026-05-06·08·12·14**, σ20 **41.64·35.79·38.61·37.60%** 로 전부 청산선 초과이며 **플래너 0주 → 1차 래퍼 632·606·594·573주**. 반론서 수치와 정확히 일치. 1차의 인과 설명("현금 순서 때문에 더 싼 K200 단이 탈락")은 **철회**.
- 정정 실험(`scripts/entry_alloc_study.py`, 저장소 반영): 플래너 매수 구간(부트스트랩→그리드→레버리지 전략→전술→총상한)을 복제하고 스위치만 달았다. **등가성 — 계획일 2,104개 중 불일치 0**, 수수료 포함 예산 위반 **0일**, 바뀐 날 중 σ20>35% **0일**. 차이 유의성은 단일 전략 구간이 아니라 **두 곡선 일수익 차이의 21일 블록 짝 재표집**으로 봤다.
- 정정 결과(전 구간/89창 정확 차이): **A1 생략→축소** 295.08%(+2.84%p)·MDD −22.61%·샤프 1.053, 67일 변화, 창 이김 51·짐 17, 짝 부트스트랩 90% 구간 +0.0014~+0.0140(양(+) 99%) → **채택 권고(강화)**. **A3 레버리지 우선** 291.67%(−0.57%p), 8일, 이김 18·짐 27 → **미채택(사유 정정 — 이득 없음)**. **A13** 294.52%(+2.28%p), 이김 28·짐 30, MDD 악화 → **미채택(A1 단독이 우월)**.
- 1차 문구 정정 8건: "MDD 변화 0"→"0.2%p 임계 초과 변화 없음"(A1 은 66창에서 −0.13~+0.18%p), "상한 +0.09%p"→관측 최댓값·결함 측정, "공동 배분 일반 기각"→순서식 휴리스틱 하나, "B 불가"→입력 추정 불가(원리적 불가 아님), "체결 품질 문제 없음"→대조군 없음, "추정 불가"→현 표본이 지지하지 않음, 분봉 연구 범위, 부트스트랩 오용. "A1 은 한 줄"→**`planner.py:284-286` 3줄 국소 변경**으로 정정.
- 테스트 결과: `pytest -q tests/` → **352 passed**(신규 3 — 등가성·예산·게이트). 한 차례 실행에서 무관한 `test_day_change_asof` 1건 실패했으나 재실행 2회·단독·짝 실행에서 재현되지 않음(공용 CI DB 간헐 실패).
- 운영 코드·파라미터 불변. 1차 문서에 정정 배너 추가.
- 문서: `docs/entry-allocation-review-r2-20260913.md`.
- Git commit: fix: correct the entry-allocation study - the leverage gate was bypassed

## [2026-09-13] fix | 진입 배분 A1 — 2차 반론 수용, 채택 근거 재정의 (r2.1, 사용자 지시)

- 2차 반론이 r2 의 성과·등가성·테스트를 재현했고(누적 +2.84%p, CAGR +0.1018%p, 창 51/17/21, 최종 체결·자산곡선 동일) 실험 무효화 결함은 없다고 판정하면서 네 가지를 지적 — **넷 다 수용**.
- ① **A1 은 정합성 수정이 아니라 배분 정책 변경**: 그리드 예산이 50/30/20 이라 1단이 현금을 넘으면 현행은 1단을 건너뛰고 더 작은 2·3단을 낸다. A1 은 1단을 축소해 채우므로 같은 현금이 **더 얕은(비싼) 가격**으로 간다. 실측 2020-09-22 `grid2 81 + lev_tact1 97` → `grid1 126`, 8.6년 체결 구성 grid1 183→185 · lev_tact1 28→**25**. 채택 사유를 "오류 제거"에서 **"이 배분 정책을 고른다"**로 변경. 얕은 단 선호는 체결률을 올리는 방향이라 `fill-rate-study` 의 함정 경계 필요.
- ② **예산 보장 범위**: r2 의 "위반 0일"은 버퍼 미제외 현금 기준이었다. 버퍼 보존 기준으로 A1 **2일·최대 1,692원**(2차 반론 실측 1,691.80원과 원 단위 일치), **현행도 1일·3,136원** — 플래너가 예약에서 수수료를 빼지 않는 **기존 성질**이지 A1 이 만든 것이 아니다. 현금 자체 초과는 0일. (r2 초안의 "58~92일"은 내 검사 오류 — 현금<버퍼라 매수가 불가능한 날까지 셌다. 매수 있는 날만 세도록 수정.)
- ③ **통계 표현**: 단일 전략 노이즈 밴드(연 ±14%p)를 차이 판단에 재사용한 문장 삭제(스스로 철회한 논리였다). "양(+) 99%"는 **재표집 중 양수 비율**이지 미래 우월·무해 확률이 아니며 89창 중 17창은 더 나빴다. 블록 길이 5/21/63/126/252일 민감도 98.7/98.6/97.1/95.6/96.0% 로 2차 반론과 일치 — 21일 선택에 의존하지 않으나 블록이 길수록 하한이 0 에 접근(+0.0001).
- ④ **A3·A13 우열 단정 철회**: 변형끼리 직접 짝 비교 — A13 − A1 관측 −0.0014, 90% 구간 −0.0064~+0.0033(양 30.7%)로 **구분되지 않는다**. "A1 단독이 우월" → "추가 복잡성을 정당화할 개선 근거가 없다". A3 − A1 은 구간 전체 음수(양 1.1%)라 상대 열위는 유효.
- 테스트 결과: `pytest -q tests/` → **355 passed**(신규 3 — 현금 0·음수·1주·수수료 경계 / 얕은 단이 깊은 단·레버리지를 대체함 / 예산 보장은 현금이지 버퍼가 아님).
- 판정: **A1 은 "검증된 버그 수정"이 아니라 "과거 표본에서 소폭 유리했던 현금 배분 정책 변경"으로 채택 후보.** 운영 코드·파라미터 불변, 플래너 수정은 미착수.
- 문서: `docs/entry-allocation-review-r2-20260913.md` §8.
- Git commit: fix: reframe A1 as an allocation policy change and correct the budget and statistics claims

## [2026-09-13] feat | 현금 부족 그리드 단을 생략 대신 축소 (A1 구현, ADR-013, 사용자 승인)

- 3차 반론의 보완 3건을 반영해 구현했다. ① `cash_left -= qty * price` 와 `orders.append(...)` 는 **원래 위치 유지** — 축소분이 현금을 먼저 차감해야 뒤 단·레버리지가 잔여에 맞춰 준다. 바뀐 것은 `continue` 분기뿐(`planner.py` 그리드 루프 3줄). ② "수수료 포함 매수가 현금을 넘지 않는다"는 **일반 보장이 아니다** — 과거 표본의 **계획가격 기준** 검사이고, 이 분기는 원금이 현금을 넘을 때만 수수료를 보며 실제 체결가·매도대금 변동은 범위 밖. ③ "체결률 상승"은 미확인이었으므로 **분모를 명시해 측정**.
- 적용 결과 — 고친 플래너로 래퍼 없이 백테스트: 누적 **295.08%**·CAGR 17.89%·MDD **−22.61%**·샤프 1.053·체결 599 로 **연구본 A1 과 소수점까지 일치**(채택 전 292.24%/−22.67%/1.049/600). 2차 반론이 요구한 마지막 채택 전 조건 충족. 복제본 등가성은 방향을 바꿔 유지(스위치 없음 = 새 플래너 2,104일 불일치 0, `legacy_skip=True` = 채택 전 규칙). 89창 짝 비교도 부호만 반전(채택 전 이김 17·짐 51).
- **주문선 체결률 측정**: 그리드 계획 2,138→2,039건·체결 234→237건으로 10.94% → **11.62%**. 분자(+3)보다 **분모(−99)** 기여가 크다(축소 단이 현금을 다 쓰면 뒤 단이 계획되지 않음). 전체 체결 600→599 로 사실상 불변 — **"체결이 늘어 유리하다"는 주장은 하지 않는다**.
- 성격: **오류 수정이 아니라 배분 정책 선택**(얕은 단 우선). 근거는 계획–실행 원리 일치(실행기는 이미 매수가능조회로 축소)와 과거 표본의 소폭 우위이며, 미래 우월성·위험 비악화는 보장하지 않는다(89창 중 17창 열위, 블록 길수록 구간 하한 0 접근). 현금버퍼(0.5%)는 보존되지 않는다 — 변경 후 2일·1,692원, 변경 전 1일·3,136원으로 **기존 성질**.
- 테스트 결과: `pytest -q tests/` → **356 passed**(복제본 등가성·축소 단 생성·예산 성질·σ20 게이트·현금 0/음수/1주/수수료 경계·정책 효과 7건 포함). 부분 실행에서 `test_signals` 1건이 실패했으나 단독·전체 실행에서 통과(공용 CI DB 간헐).
- 문서: [ADR-013](adr/013-grid-rung-clip.md) 신설, `features/feature-strategy-engine.md` §5.5·변경 이력, `docs/entry-allocation-review-r2-20260913.md` §9.
- Git commit: feat: clip the cash-short grid rung instead of dropping it

## [2026-09-13] research | 애프터마켓 야간 정보 활용 제안 검토 — 상한 측정 후 미채택 (사용자 지시)

- 제출된 분석서(① 종가를 야간 추정가로 교체 비권고 ② 구성종목 야간지표 A_t 를 보조 신호로 검증 ③ 모의 병행 기록)의 코드 사실 주장은 **전부 일치** — 주문표는 저장된 ETF 일봉만 읽고, 야간·구성종목 입력 경로가 없으며, 20:10 주식 수집은 `AFTER_MARKET_TYPES=("STOCK",)` 로 ETF 를 제외하고, 09:01 실행기는 실제 시가로 갭 취소를 판정한다.
- **상한 측정으로 판정했다.** 야간 정보의 최선은 "다음 날 시가를 미리 아는 것"인데 ⓐ 우리 종목은 애프터마켓 제외라 16:00~20:00 에 **실행할 수 없고** ⓑ 09:01 실행기가 **실제 시가를 이미 본다**. 따라서 실제 시가로 주문표를 고친 성과가 모든 야간 신호의 상한이며, 야간 자료 없이 측정 가능하다.
- 결과(2018-01-26~2026-09-10, 현행 295.08%): **S1 그리드 앵커를 시가로 → 247.91%**(−47.17%p, 체결 599→459, 창 39:47, 짝 부트스트랩 양(+) 12%), S2 시가로 수량 재평가 → 293.64%(무차별, 창 45:39), **O1 "그날 음봉이면 매수 취소"(방향 완전 예지) → 228.83%**(−66.25%p, 창 33:52) — **그리드는 하락일을 사는 전략이라 "나쁜 날을 피하는" 정보는 가치가 음수다.** 도움이 되는 유일한 예지는 O2 그날 **저가** 지정(392.07%)인데 이는 장중 정보라 전날 밤에 존재하지 않는다.
- 밤갭 분포(2,373일): 평균 +0.081% · 중앙 +0.101% · |갭| 중앙 **0.399%** · 90% 1.350% · 1% 초과 16.6%. 그리드 간격 0.8~2.5% 대비 대부분의 밤은 지정가 한 칸도 못 움직인다.
- 데이터 관점에서도 착수 불가 — 지수 구성종목·비중 이력 없음(DB 국내 주식 3종목), 구성종목 야간 시세 수집 경로 없음, 새 제도 표본 0일.
- **판정: ①·② 미채택, ③ 보류, 야간 방향 표시도 보류**(표시가 수동 개입을 유도하면 측정과 반대로 움직이기 쉽다). 2026-09-06 검토의 "공식 수정 없음" 유지. **다시 볼 시점은 11월 NXT ETF 거래 개시** — 그때는 우리 종목이 08:00~20:00 에 거래돼 "실행 불가" 전제가 깨진다.
- 내일 실제 영향은 RAVG 가 아니라 **일지 보유 주식**이며 20:10 재수집 · 20:15 체결 동기화 · 20:20 스냅샷 재계산이 이미 구현돼 있다(2026-09-06 권고분). 미확인은 KIS 예약주문 접수 창 변경 여부 — 9/14 첫 주 실제 접수로 확인.
- 테스트 결과: 코드·파라미터 변경 없음. 측정 `scripts/open_anchor_study.py`(저장소 반영).
- 문서: `docs/aftermarket-night-signal-review-20260913.md`.
- Git commit: docs: measure the ceiling of an overnight signal - even perfect open knowledge does not help

## [2026-09-13] research | 야간 충격(전쟁 등) 조기 반응의 가치 측정 — 미채택, 꼬리 표본 한계 명시 (사용자 지적)

- 지적: "애프터마켓 시간에 외부 충격이 터지면 다음날 시장에 영향이 있을 텐데 애프터마켓을 제외하는 건 문제 아닌가." **구조적으로 옳다** — 현행은 T일 저녁 충격 → T+1 09:01 갭 필터로 **매수만** 취소(매도 계획 없음) → T+1 저녁 재계산 → T+2 09:01 축소 실행으로 **두 장 뒤** 반응한다.
- 다만 애프터마켓 정보의 최선은 이를 **한 장 앞당기는 것**(T+1 시가 매도)뿐이다 — 우리 종목은 애프터마켓 제외라 T일 밤에 팔 수 없다. 그리고 **애프터마켓은 충격을 예고하지 않는다 — 일어난 뒤의 가격을 보여줄 뿐**이라 거기서 파는 것이 곧 T+1 시가 매도다.
- 충격 갭 실측: ≤ 종가−1.0×ATR 65일(3.1%, 평균 −2.58%) · 1.5×ATR 21일(1.0%, −3.77%) · 2.0×ATR 5일 · 최악 −9.17%. **갭 뒤 그날 종가는 평균 −0.08%~+0.48%, 5일 뒤 −0.05%~−1.03%** — 패닉은 갭에 반영되고 이후는 평평하다.
- 조기 반응(충격일 T+1 시가에 σ·E·레짐 재계산분을 시장가 매도): k=1.0 300.05%(+4.97%p, MDD −0.51%p, 발동 14회)이나 **짝 부트스트랩 구간 −0.0089~+0.0389 로 0 을 걸친다**(양 79%). k=1.5 291.75%(−3.33%p, 발동 5회), k=2.0 291.96%, k=3.0 발동 0. 위기 구간도 거의 동일(2020 코로나 −10.99→−10.86%, 2022·2024-08 **동일**). 89창 k=1.5 평균 −0.094%p·이김 11:짐 13, MDD 개선 19창. → **미채택**.
- 발동 사례에서 레짐이 실제로 바뀐 것은 2020-03-09 한 번뿐, 나머지는 σ 상승에 따른 E 하향 — **하루 뒤 어차피 일어나는 축소**이며 그 하루가 평평하다.
- 남기는 것 둘: ① 우리는 충격에 두 장 뒤에 반응한다(선택의 결과이며 이 표본에서 비용 미확인). **표본에 진짜 전쟁급 사건이 없다 — 1.5×ATR 초과 21건으로 꼬리를 판정할 수 없다**는 한계 명시 ② **진짜 방어는 정보 타이밍이 아니라 보유량** — 레짐 사다리(1.30/0.65/0.20)와 σ20>35% 청산이 미리 줄여 놓는 것이 충격 손실의 크기를 정한다.
- 11월 NXT ETF 거래 개시 때 성격이 바뀐다(우리 종목이 08:00~20:00 거래 → "밤에 팔 수 없다" 전제가 깨짐). 그때는 **야간 호가·체결 자료 수집부터** 시작한다 — 야간 유동성이 얇아 매도 슬리피지가 커지는 점도 함께 본다.
- 테스트 결과: 코드·파라미터 변경 없음. 측정 `scripts/shock_reaction_study.py`(저장소 반영).
- 문서: `docs/aftermarket-night-signal-review-20260913.md` §8.
- Git commit: docs: measure early shock reaction - the gap prices the panic, the next day is flat

## [2026-09-13] research | NXT 야간장 매매 검토 — 시가 갭 이점을 반납하므로 미채택 (사용자 질문)

- 질문: "NXT 장에 거래가 개시되면 우리도 야간장에 매매하는 게 더 효율적인가." **아니다 — 유동성 때문이 아니라 체결 방식 때문이다.**
- 현행은 전날 저녁 지정가를 걸고 **09:00 단일가가 그 아래에서 체결되면 차액을 받는다**(시가 ≤ 지정가 → 시가 체결). 매도도 대칭(갭 상승 시 익절가가 아닌 시가). 실측: 매수 체결 243건 중 **시가 체결 44건(18.1%)**, 시가가 지정가보다 쌌던 주문 61건의 이점 **평균 0.86% · 중앙 0.44% · 90분위 2.26% · 최대 4.95%**.
- 야간 연속거래는 가격이 지정가를 **지나가는 순간** 체결되므로 이 이점이 사라진다. 그 상한을 측정(체결 규칙만 교체): **N1 야간 매수·매도 265.09%**(−30%p, MDD −22.61→−25.14%, 샤프 1.053→0.997, 부트스트랩 90% 구간 −0.1285~−0.0384 **양 0%**, 89창 **1:88**), N2 매수만 289.21%(−5.9%p, 3:86), **N3 매도만 268.45%**(−26.6%p, 1:65) — **매도 쪽 손실이 더 크다**(익절이 갭 상승을 받는 이점).
- 야간 슬리피지 민감도: 시장가 0.2%/0.3%/0.6% 에서 260.02%/254.75%/242.15%. NAV 괴리·부분체결도 같은 방향.
- **판정: 야간장 매매 미채택.** 우리 전략은 **단일가에 지정가를 걸어 두고 갭을 받는 구조**이고 연속거래는 그 이점을 반납한다. 09:00 KRX 단일가는 거래가 가장 두꺼워 시장가 줄 체결도 가장 좋다. **11월 NXT 상장 후에도 신호 앵커·실행 시점 불변**(ASSUMPTIONS 2026-09-06 유지), NXT 에서 할 일은 자료 수집·관찰뿐.
- 한계: "지정가를 지나가면 지정가 체결"은 근사다. 실제 야간 체결은 호가 두께에 따라 더 나쁠 수 있고 드물게 더 나을 수 있으나, 89창 중 88창 열세는 근사 오차로 뒤집히지 않는다.
- 테스트 결과: 코드·파라미터 변경 없음. 측정 `scripts/night_session_study.py`(저장소 반영).
- 문서: `docs/aftermarket-night-signal-review-20260913.md` §9.
- Git commit: docs: night-session trading gives up the opening-gap fill advantage

## [2026-09-13] research | 11월 NXT 관측의 **사전 등록 기준** — 무엇이 나오면 판단을 바꾸는가 (사용자 질문)

- 질문: "자료 수집·관찰해서 어떤 결론이 나오면 환영할 만한가." 기각 근거가 관측 가능한 사실에 걸려 있으므로 **기준을 자료가 나오기 전에 못박았다**(사후 확정은 합리화가 된다).
- **야간장 실행 전환 조건(§9 뒤집기)** — 넷 모두 충족 시에만 재계산: ① 09:00 시가 체결 비중 18.1% → **<5%** ② 시가가 지정가보다 쌌던 폭 평균 0.86% → **<0.2%** ③ 야간 시장가 슬리피지 정규장 0.1%+0.1%p 이내 ④ 1회 주문이 스프레드를 2틱 이상 벌리지 않음. **시가 이점이 커지면 야간 매매는 지금보다 더 나쁘다.**
- **야간 비상청산만 여는 조건(§8 의 절반)** — 성격이 수익이 아니라 꼬리 위험이라 기준을 따로: 청산 사유 발생일에 야간 체결가가 다음 날 09:00 시가보다 평균 유리 · 야간 슬리피지 0.3% 이내 · 표본 5건 이상. 충족 시 **강제청산 줄에 한해** 검토(그리드·익절·진입은 대상 외).
- **충격 정보 재검토 조건(§8 뒤집기)**: 충격 갭(≤−1.5×ATR) 다음 그날 종가가 현재 **+0.04%**(21건) → **≤ −0.5%** 이고 표본 20건 이상, 5일 뒤 ≤ −1.5%. "패닉이 갭에 다 반영된다"가 깨져야 조기 반응이 값을 갖는다.
- **전략 변경 없이 환영할 결과**: 밤갭 축소(현재 |갭| 중앙 0.399%·1% 초과 16.6%) · 갭 취소 발동 감소(8.6년 21건) · NXT 체결이 원장에 정상 기록되는지 첫 주 확인(**틀리면 즉시 고칠 결함**).
- 기본 시나리오는 **"아무것도 바뀌지 않는다"** 이며 그것도 좋은 결과다 — 관측 비용은 수집 배치 하나, 얻는 것은 구조 유효성 확인. 위험한 것은 관측 없이 "야간장이 열렸으니 뭔가 해야 한다"고 움직이는 쪽이다.
- 테스트 결과: 코드·파라미터 변경 없음(문서만).
- 문서: `docs/aftermarket-night-signal-review-20260913.md` §10.
- Git commit: docs: pre-register what November observation would have to show

## [2026-09-13] research | 보유분 시작 포트의 익절 사다리 검토 — 채택 권고(미구현) (사용자 지시)

- 지적 확인: `보유분 입력`으로 시작하면 종목당 로트가 하나라 중립장 익절이 **한 가격·전량** 한 줄로 나가고(`planner.plan` 의 core 로트 익절 = 종가×(1+Grid)), 그 밖의 매도는 축소(밴드 초과 시 시장가)뿐이다. 분할이 없다는 지적 그대로다.
- 2026-09-03 검토와의 차이: 그때 기각한 것은 **평단 역계산 로트 재구성**(평단<종가면 즉시 전량 매도 함정)이고, 이번 제안은 **평단을 건드리지 않고 수량만 사다리로** 나누는 것이라 기각 사유에 걸리지 않는다. 다만 그 검토의 측정("정본 모델도 익절일 중앙 100% 매도")은 재확인했다 — 익절일 32일·전량 비중 66%.
- 측정(시작 현금 40%+K200 60%, 시작일 89개×1년, core 익절만 교체): **L1 사다리 50/30/20·1×/2×/3×Grid** 평균 +0.252%p·중앙 +0.080%p·**이김 46·짐 18·동일 25**, MDD −12.35%→−12.28%, 전량 비중 57%→42%. L2 균등 1/3 은 +0.350%p(같은 46:18)이나 1× 에 1/3 만 배정해 덜 파는 쪽으로 더 기움. L3 촘촘(1·1.5·2×) +0.168%p(39:18). **L4 절반만 팔고 나머지 영구 보유는 34:30 으로 승률이 크게 낮다** — 윗단을 실제로 거는 것이 "덜 팔기"보다 낫다는 뜻.
- 보유 비중 30%/60%/100% 에서 부호·승패 동일(크기만 비례) — L1 +0.243/+0.252/+0.248%p.
- 기전: 다음날 고가가 종가×(1+k·Grid)에 닿는 빈도 **1× 28.4% · 2× 5.3% · 3× 1.1%** — 윗단은 드물게만 체결되므로 실질 효과는 "작은 급등에 절반만 팔고 나머지는 들고 간다".
- 적용 범위는 **core 로트 한정**(익절가를 '오늘 종가'에서 받는 로트 = 보유분 입력분, 상승장 매수 후 전환 전). 자기 체결가에 묶인 그리드 로트는 정본 규칙이라 제외.
- **채택 권고하되 구현은 지시 대기.** 한계: 연 +0.25%p 는 노이즈 밴드 안이고 18창에서 열세, 표본의 상승 편향이 "덜 파는" 변형에 유리할 소지(L4 비교로 일부만 걷어냄), 익절가가 오늘 종가 기준이라는 근사는 그대로다.
- 테스트 결과: 코드·파라미터 변경 없음. 측정 `scripts/entry_holding_tp_study.py`(저장소 반영).
- 문서: `docs/entry-holding-tp-ladder-20260913.md`.
- Git commit: docs: measure a take-profit ladder for entered holdings

## [2026-09-13] fix | 소급 태깅이 전량 청산을 유발할 뻔한 결함 수정 + 2차 사다리 보고서 검토 (사용자 지시)

- **결함(오늘 내가 만든 것)**: `scripts/retag_legacy_lots.py` 는 원가 기준 익절가(`체결가×(1+계획일 Grid)`)를 붙이는데, **오래전 싸게 산 보유분**은 그 익절가가 이미 한참 지나갔다. 지정가 매도는 시장가보다 낮으면 **즉시 체결**되므로, 그대로 실행했다면 다음 계획에서 그 포트의 K200 이 **전량 시장가 매도**됐다. 개발 DB 실측 **56건 중 53건** 해당(예: 취득가 70,000 → 익절가 70,880, 최근 종가 **111,865**). 2026-08-28·09-03 검토가 "평단 역계산 → 즉시 전량 매도"로 경고한 함정을 반대편에서 재현한 셈.
- **조치**: 재생한 익절가가 최근 종가 이하이면 `core`(익절가 없음)로 둔다 — 태그 전 폴백과 같은 동작이라 **소급 태깅이 매매를 바꾸지 않는다**는 원칙이 지켜진다. 회귀 테스트 `test_retag_never_assigns_a_take_profit_that_is_already_passed` 추가. **운영 미반영 상태에서 발견**(dry-run 만 수행했음).
- 2차 보고서 코드 주장 검증 — 셋 다 맞다: 미태그 로트는 "로트 부재"가 아니라 **태그·개별 익절가 부재**(`lots.rebuild_lots`), 같은 익절가는 **표시용으로 병합**(`signals.py`), 리밸런싱 매도는 **보유액>목표액** 때문.
- **"P₀·g₀ 고정 + 단별 잔여 유지" 안은 유보**: 측정 추가 결과 평균은 비슷하나(+0.294%p) **승률 33:31**(매일 재계산 L1 은 46:18)이고 **전량 비중 55%로 현행(57%)과 거의 같아 문제를 해결하지 못한다**. 기전 — 고정 사다리는 P₀ 아래로 내려가면 오래 안 팔리다가 회복 시 세 단이 한꺼번에 체결된다. 보고서의 "매일 재계산하면 상승 시 체결 안 된다"는 우려는 이 자료에서 반대로 나타난다(익절일 3.9 vs 3.3) — 1 Grid 는 일중 변동폭 안에 자주 들어온다. 다만 **진행 상태 가독성** 지적은 유효하므로 화면 표시로 푼다.
- **"분할 매도 ≠ 익절" 지적 수용, 별건 분리**: 현행 미태그 익절가는 최근 종가 기준이라 평단과 무관 — 평단 45,000 인 보유분에 40,400 익절이 나갈 수 있다(손실 실현을 익절로 호칭). 이 지적이 위 결함 발견의 단서였다. 라벨 변경 또는 평단+수수료 하한 도입은 **반대편 함정**(체결 지연·즉시 매도)을 함께 검증해야 하므로 별도 과제.
- 수정된 권고: 사다리는 **L1(매일 재계산 50/30/20 · 1·2·3×Grid)**, 주문표에 `익절 1/3·2/3·3/3` 과 단별 진행 표시. 구현은 지시 대기.
...
## [2026-09-13] feat | 코어 로트 익절을 사다리로 (ADR-014, 사용자 승인 "사다리 구현을 진행하세요")

- 구현: `planner.plan()` 의 core 익절 분기를 `_core_tp_ladder()` 로 교체. 가격 `종가×(1+k·Grid)`(k=1..`grid_steps`), 수량은 `grid_weights`(50/30/20)를 **최대잔여법**으로 정수 배분 — 합계 = 축소 선점을 뺀 available, 1주면 **1단**(가장 가까운 가격), 빈 단 없음. **그리드 로트는 대상 외**(체결가에 묶인 익절가가 정본 규칙). 축소 FIFO 선점·밴드·레짐 전환 불변.
- **구현본이 연구본과 일치**: 시작일 89개×1년 짝 비교 구현본 +0.245%p·이김 46·짐 18·동일 25 vs 연구본 L1 +0.250%p·46:18:25(차이는 정수 배분 방식). 전량 매도 비중 57%→42%, MDD 평균 −12.35%→−12.28%.
- 기전: 다음날 고가가 종가×(1+k·Grid) 도달 빈도 1× **28.4%** · 2× 5.3% · 3× 1.1% — 윗단은 드물게만 체결되므로 실질 효과는 "작은 급등에 절반만 팔고 나머지는 들고 간다". "덜 파는 것"만이 이유는 아니다(절반 팔고 영구 보유는 34:30).
- 미채택 대안: 기준가·수량 모두 고정(2차 보고서안) 32:31·전량 비중 55%로 문제 미해결 / 촘촘 사다리(1·1.5·2×) 39:18 / **수량 고정+가격 매일 갱신(L7) 평균 +0.422%p·최악 −0.464%p 로 더 낫지만 단별 잔여 저장이 필요** — 플래너가 원장에서 매일 재계산하는 순수 함수라 상태 도입은 부분체결·재시작·수동매도마다 새 실패 모드를 만든다. 연 +0.18%p 가 그 복잡성을 정당화하지 않음(재검토 조건 ADR 에 기록).
- 한계 명시: 이득 연 +0.25%p 는 노이즈 밴드 안이고 **18창에서 열세**. 채택 근거의 무게는 성과가 아니라 "근사치인 익절가를 한 가격에 몰지 않는다"는 행동의 타당성. 익절가가 오늘 종가 기준이라는 근사와 평단과의 관계(평단 미만 매도를 '익절'로 호칭)는 **별건**.
- 화면: 익절이 최대 3줄(가격이 달라 표시 병합 안 됨). 주문 종류는 `tp` 유지 — 체결 귀속(`lots.sell_tag`)·실행기 `line_key` 불변. 단별 라벨은 넣지 않음(귀속 규칙까지 건드려야 함).
- 테스트 결과: `pytest -q tests/` → **361 passed**(신규 3, 기존 전환일 테스트 1건을 사다리 기준으로 갱신 — 전환일 사다리·배분 경계·축소 선점 우선·그리드 로트 단일 유지).
- 문서: [ADR-014](adr/014-core-tp-ladder.md) 신설, `features/feature-strategy-engine.md` §5.5·변경 이력, `docs/entry-holding-tp-ladder-20260913.md` §9.
- Git commit: feat: split the core-lot take-profit into a ladder

## [2026-09-13] research | 매도 사다리 가중치 비교 — 50/30/20 vs 균등 1/3 (사용자 질문)

- 가중치를 **한 축**으로 쓸어 현행(50/30/20) 대비 짝 비교(시작일 89개×1년). 1단 비중 70%→0% 순: 평균 차이 −0.098 / (기준) / +0.107(균등) / +0.211 / +0.315 / +0.423%p, 승률 18:46 / — / **44:20** / **45:19** / 36:28 / 35:29.
- **평균과 승률이 다른 말을 한다**: 평균은 1단 비중이 낮을수록 **단조 증가**(덜 팔수록 좋게 나옴 = 표본의 상승 편향 신호)인데, 승률은 20~33% 에서 꼭대기를 찍고 10%·0% 에서 꺾인다 — **1단에서 아예 안 파는 것은 나쁘다**. 중앙값은 거의 전부 0.000%p 라 평균 차이는 꼬리 몇 창이 만든다.
- 매수와 대칭이 아닌 이유: 매수 50/30/20 은 **체결 확률에 예산을 정렬**한 것인데(1단 25%·2단 6.3%·3단 1.45%), **매수는 체결이 목적이고 매도는 체결이 비용**이다(계속 오를 자산을 넘기는 기회비용). 그래서 매도에서 1단 최대 배정은 최적이 아니지만 반대 극단도 아니다.
- **판정: 지금은 50/30/20 유지 권고.** 균등 1/3 은 평균 +0.107%p·승률 44:20 으로 근거가 있으나 ① 연 +0.11%p 는 노이즈 밴드 안이고 중앙값 0.000%p ② 바꾸면 `core_tp_weights` 상수가 새로 생겨 "매수 그리드의 거울"이라는 한 줄 설명이 깨진다 ③ 단조성 때문에 가중치를 고르는 행위 자체가 상승 편향을 고르는 것이 될 수 있다. 바꾼다면 균등 1/3(20/30/50 보다 보수적)이고, 판단은 성과가 아니라 "매도는 체결이 비용"이라는 논거에 걸어야 한다.
...
## [2026-09-14] research | 익절이 보유 전량을 덮는 것이 맞는가 — 상한 3종 측정 후 현행 유지 (사용자 질문)

- 질문: 400주 보유(목표 65% 대비 실제 52% **미달**)인데 익절 사다리가 400주 **전량**을 덮고 같은 날 그리드는 51주를 산다. 맞는 공식인가.
- 측정(시작일 89개×1년, 현행 = 상한 없음): **목표 초과분만**(미달이면 익절 0) +0.005%p·**이김 27:짐 38**·최악 **−8.407%p** / 보유의 50%까지 −0.102%p·36:28 / 목표의 50% 남기기 −0.238%p·26:38. **가장 직관적인 안이 가장 자주 지고 최악 창도 가장 깊다.** → **현행(상한 없음) 유지.**
- 이유: 익절과 축소는 **다른 축**이다 — 익절은 "가격이 +Grid 올랐나"(왕복의 매도 다리), 축소는 "보유액이 목표를 밴드 넘었나"(노출 관리). 주문표의 `106,990 매수 51주` 와 `112,480 매도 200주` 는 모순이 아니라 왕복의 양 끝이고 둘 다 체결되면 5.1% 를 번다. 목표 미달은 "그리드가 채우는 중"이라는 뜻이며, 그 사이 급등을 실현하고 더 낮은 가격에 되담는 것이 중립장 수익원이다.
- 전량이 한 번에 나가려면 사다리 3단이 하루에 모두 닿아야 한다 — 도달 빈도 1× 28.4% · 2× 5.3% · **3× 1.1%**. 사다리 도입으로 전량(≥99%) 매도 비중은 57%→42% 로 이미 내려갔다.
- 설계를 바꾸지 않는 대안: **무인 매도 끄기**(해당 포트가 이미 그 상태 — 익절 3줄이 `수동` 표시). 제안으로만 두고 원하는 수량만 체결 등록하면 되며, 2026-09-03 검토의 "부분 익절 운용은 허용되며 안전하다"와 같다.
