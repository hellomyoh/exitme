...
- 자산 추이 라인(1M/3M/6M/1Y/전체) — **총자산 + 포트별 다선**(KRW 포트, 2026-09-02) + **매매일지 합계 파선**(2026-09-10), 일간 손익 캘린더 히트맵
  ...
  - 매매일지는 **일지별 한 줄씩** (0028, 2026-09-12 지시). 원천은 `journal_snapshots`(일지·일자 단위, 포트의 `portfolio_snapshots` 와 같은 자리)이며 사용자 스냅샷 계산 시 함께 적재한다. 값은 총자산에 넣는 평가액(실전매매와 겹쳐 제외된 종목 제외, 전 종목 시세가 없으면 취득원가), `counted` 가 false 인 날은 선에서 뺀다. 일지 스냅샷이 하나도 없는 사용자는 종전처럼 사용자 합계(`asset_snapshots.journal`) 한 줄로 폴백한다(집계 이전 NULL 날짜는 건너뛴다).
  - **과거 구간은 근사** — `scripts/backfill_journal_snapshots.py` 가 기록(일자·수량·단가·종목코드)과 그날 종가로 되살리고 `approx=True` 로 표시한다. 되살리지 못하는 둘: 당시 실전매매와의 중복 제외 상태, 시세가 없는 종목(취득원가로 둔다). 차트 범례 아래에 근사임을 알린다. 이미 있는 배치분(정확값)은 덮지 않는다(`--overwrite-approx` 로만)
