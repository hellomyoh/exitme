...
- **확정봉 가드 (2026-09-07)**: 일봉은 **확정된 봉만 적재**한다 — 미래 날짜 거부, 오늘 날짜 봉은 해당 시장 정규장 마감(KR 15:30 KST / US 16:00 ET) 이후에만 저장. 거부 시 `IngestResult.rejected` 증가 + 경고 로그. 판정은 `services/ingest.bar_is_final()` 한 곳이며 네 적재 경로(daily_ingest·매매일지 일봉 보충·seed·ingest_us_daily)가 모두 이를 통과한다. 근거: `on_conflict_do_nothing`(원본 불변, ADR-002)이라 미완성 봉이 한 번 들어가면 마감 후 진짜 종가가 영구히 막히고 주문표 가격 전체가 오염된다([NOTES](../NOTES.md) 2026-09-07 사고).
- **애프터마켓 반영 (2026-09-14)**: 마감 시각을 종목 종류로 나눈다 — 국내 ETF·ETN 15:30, 국내 주식 20:00(`ingest.KR_STOCK_CLOSE`), 미국 16:00 ET. 수집도 나뉜다: 16:05 `scope="regular"`(국내 ETF·미국), 20:10 `scope="stocks"`(국내 주식).

- 시딩: **KIS 일봉 API 1순위**(10년 ≈ 코드당 26회 호출), pykrx는 폴백 — KRX가 pykrx 요청을 차단함이 확인됨([NOTES.md](../NOTES.md) KRX 절, 2026-08-28 명세 갱신). (종목, 연도) 체크포인트로 이어받기(`ON CONFLICT DO NOTHING`), **빈 응답은 실패로 처리**(체크포인트 미기록, 조용한 0건 시딩 금지), 완료 리포트(적재 행수·실패 목록).
...
- **1분봉 (2026-08-28 채팅 지시 반영)**: KIS 주식일별분봉조회(`FHKST03010230`, 보관 한도 약 1년·호출당 120건·시간 커서 페이지네이션)로 수집.
  **증분 원칙 — DB 최신 ts 이후만 API 호출**(최신 일자는 재조회하되 ON CONFLICT 멱등): 백테스트는 DB만 읽으므로 반복 실행 시 API 를 재호출하지 않는다.
  소급 수집은 `scripts.seed_minutes`(하루 단위 commit 이어받기), 당일분은 daily_ingest 배치가 자동 수집.
- KIS 유량 제한 대응: 호출 간 최소 간격 0.15s + 500 응답 지수 백오프 재시도. 토큰은 Redis 공유 캐시(발급 분당 1회 제한 대응).
