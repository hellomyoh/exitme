...
- **직접 주문 (ADR-011, 2026-09-10 지시)**: 연결 계좌가 있으면 앱에서 KIS 정규 주문을 바로 낸다 — `POST /portfolio/{pid}/orders/manual {code, side, qty, price|null}`(price 없으면 시장가). 기록은 `BrokerOrder(mode="manual")` 라 취소·체결 확정·표시가 무인과 같은 경로. 통제: 연결 계좌만 · 장중 09:00~15:20 · 매수는 매수가능조회, 매도는 잔고로 **사전 검증하고 초과면 거부**(수량을 줄이지 않는다) · 같은 주문 5초 잠금 · `order.manual` 로그·알림. 표기는 `👤 직접 발주됨`(무인은 🤖), 주문표에 없는 자유 주문은 아래 '직접 주문 · 오늘 낸 주문' 카드에서 본다. 매매일지도 같은 표(`broker_orders.journal_id`, 0026)와 같은 본체(`place_manual_kis_order`·`cancel_regular_order`)를 쓴다 — `GET/POST /mjournals/{jid}/orders…`.
