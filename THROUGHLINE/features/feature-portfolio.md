...
- **플래그 가시성 (2026-09-08 지시 "설정을 바꿔도 주문표에 변화가 없다")**: 상태 줄에 `매수 ON`·`매도 OFF` 배지와 "09:01 발주 예정 n줄 · 수동 m줄" 요약. 표의 '무인' 열은 실행 전에도 줄별로 `🤖 09:01 발주`(켜진 방향) / `수동`(꺼진 방향, 롤오버에 이유) / `수동(취소)` / `—` — 실행 전에는 같은 줄의 옛 행(전환 정리로 취소된 승인 행 등)을 무시하고 예고만 보인다(2026-09-08). 실행 후에는 실제 결과.
