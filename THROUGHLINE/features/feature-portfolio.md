...
- **모바일 (2026-09-10 점검)**: 본문 컨테이너가 가로 넘침을 **잘라내므로**(`shell.tsx`, 툴팁이 페이지를 밀지 않게) 스크롤 래퍼가 없는 요소는 넘치면 손댈 수 없다. 규칙: ① 표는 반드시 `overflow-x-auto` 래퍼 안에 ② `CardTitle` 의 오른쪽 액션은 좁으면 아랫줄로 내려가고 그 안에서도 줄바꿈(`flex-wrap` + `sm:whitespace-nowrap`) ③ 탭·버튼 묶음은 설명 문구를 `hidden sm:inline` 으로 숨기고 `flex-wrap` 을 준다.
- **입력 검증 표기 (2026-09-10 지시 "누락된 입력을 알 수 있게, 모든 폼에 동일하게")**: 공통 훅 `lib/form.tsx` `useFieldErrors()` — `validate({칸: 사유|""})` 가 표시를 켜고 사유를 이어 붙여 돌려주며, `cls(k)` 로 붉은 테두리·`msg(k)` 로 칸 아래 사유·`clear(k)` 로 입력 시 해제. **버튼을 조용히 비활성으로 두지 않고** 눌렀을 때 무엇이 빠졌는지 보이게 한다. 적용: 실전매매 거래 입력 3탭 · 매매일지 기록·주문·새 일지 · 설정 증권사 계좌 등록/조회·알림 · 시뮬레이터 기간·자본금 · 로그인.
- **직접 주문 (ADR-011, 2026-09-10 지시)**: 연결 계좌가 있으면 앱에서 KIS 정규 주문을 바로 낸다 — `POST /portfolio/{pid}/orders/manual {code, side, qty, price|null}`(price 없으면 시장가). 기록은 `BrokerOrder(mode="manual")` 라 취소·체결 확정·표시가 무인과 같은 경로. 통제: 연결 계좌만 · 장중 09:00~15:20 · 매수는 매수가능조회, 매도는 잔고로 **사전 검증하고 초과면 거부**(수량을 줄이지 않는다) · 같은 주문 5초 잠금 · `order.manual` 로그·알림. 표기는 `👤 직접 발주됨`(무인은 🤖). **입력은 한 섹션 (2026-09-10 지시 "주문유형으로 탭 구분")**: 주문표 아래 '거래 입력' 카드에 탭 3개 — `✍️ 체결 기록`(장부에만 적기 — 종전 '체결·입출금 등록' 접힘 영역을 대체) · `💰 입출금` · `👤 증권사 주문`(실제 발주, 경고색·계좌 배지, 계좌 미연결·국내 아님이면 비활성). 주문표 줄의 '체결 등록' 버튼은 체결 탭을 열고 값을 채운다. 카드 아래에 '오늘 낸 주문'(🤖 무인 / 👤 직접 / 예약)을 상태·취소와 함께 둔다. 매매일지도 같은 구성(탭 2개: 기록만 · 증권사 주문). 매매일지도 같은 표(`broker_orders.journal_id`, 0026)와 같은 본체(`place_manual_kis_order`·`cancel_regular_order`)를 쓴다 — `GET/POST /mjournals/{jid}/orders…`.
- **무인 매매 (ADR-009)**: 발주 경로는 HTS 직접, 09:01 무인 실행, 앱의 직접 주문(ADR-011). 무인은 **증권사 계좌의 플래그** `auto_exec = {buy, sell, daily_buy_cap_pct}` 로만 켜고 끈다(승인 단계·주문표 버튼 없음). 주문표 상단에 상태 한 줄(수동/대기/실행 중/완료/취소됨/정지/경고)과 '이번 실행일 무인 취소 → 수동'(09:00 전 건너뛰기·되돌리기 / 09:01 후 살아 있는 주문 취소) 버튼만 둔다. 설정에서 플래그를 끄면 그 방향의 오늘 미체결 무인 주문을 즉시 취소한다(사용자 결정). 실행 뒤 실패(KIS 오류)·생략 줄이 있으면 '무인' 열 헤더의 `재시도`(장중 09:00~15:20)로 그 줄만 같은 절차로 다시 발주한다 — 갭 생략·발주된 줄은 대상 외, 이전 행은 기록으로 남는다(2026-09-09). 09:01/09:15 실행이 09:30 을 넘겨 도착하면(스케줄러 중단 뒤 따라잡기) 발주하지 않고 "발주 안 함 — 09:30 지연 상한 초과"로 기록한다(2026-09-09).
...
- **체결의 전략 태그 (2026-09-12, 감사 A1·A2, 0027)**: `POST /positions` 가 선택 필드 `strategy_kind`(주문표 줄의 종류: boot·grid1~3·lev_*·tp·reduce…)·`plan_grid`·`plan_regime`·`plan_price`(매도의 주문표 지정가)를 받아 서버가 `lot_kind`·`tp_price` 를 만든다(`lots.lot_tag`/`sell_tag`) — 국내 포트의 buy/sell 만, 임의 입력은 NULL. 화면의 '체결 등록' 버튼은 줄의 종류와 그날 `indicators.grid`·`regime` 을 기억해 두고 **종목·방향이 그대로일 때만** 동봉한다(사용자가 바꾸면 태그 없이 등록). `import-fills`(마감 후 배치 포함)는 체결의 주문번호로 `broker_orders` 를 찾아 같은 규칙으로 태그한다 — 무인·예약·직접 주문 모두 발주 행에 `plan_grid·plan_regime` 을 적어 두기 때문. 재구성(`_state_before`)은 매도를 귀속대로 소진한다(익절 → 그 익절가 로트 먼저). 보유·평단·실현손익 표시(`PositionLot`)는 종전 FIFO 회계 그대로 — 전략 상태 재구성과 손익 회계는 다른 축이다.
