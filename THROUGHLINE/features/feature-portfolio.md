...
- **무인 매매 (ADR-009)**: 발주 경로는 HTS 직접 또는 09:01 무인 실행뿐. 무인은 **증권사 계좌의 플래그** `auto_exec = {buy, sell, daily_buy_cap_pct}` 로만 켜고 끈다(승인 단계·주문표 버튼 없음). 주문표 상단에 상태 한 줄(수동/대기/실행 중/완료/취소됨/정지/경고)과 '이번 실행일 무인 취소 → 수동'(09:00 전 건너뛰기·되돌리기 / 09:01 후 살아 있는 주문 취소) 버튼만 둔다. 설정에서 플래그를 끄면 그 방향의 오늘 미체결 무인 주문을 즉시 취소한다(사용자 결정). 실행 뒤 실패(KIS 오류)·생략 줄이 있으면 '무인' 열 헤더의 `재시도`(장중 09:00~15:20)로 그 줄만 같은 절차로 다시 발주한다 — 갭 생략·발주된 줄은 대상 외, 이전 행은 기록으로 남는다(2026-09-09).
...
- **무인 상태 표시 (2026-09-08 상태 줄 → 2026-09-09 박스 제거 지시)**: 붉은 박스 대신 주문표 **제목 오른쪽 칩** 한 줄 — `🤖 09:01 발주 예정 · 매수 ON · 매도 OFF` / `✅ 발주 3건` / `⛔ 정지 — 사유 [다시 켜기]` / `⚠️ 09:01 기록 없음`(정지·기록 없음만 붉게, 상세·계좌·발주 n줄·수동 m줄은 롤오버). **'이번 실행일 무인 취소'는 '무인' 열 헤더의 `취소` 링크**(취소 뒤 09:00 전이면 `되돌리기`), 헤더 ⓘ 에 설명. 실행 뒤에는 살아 있는 주문이 있을 때만 `취소`, 실패·생략 줄이 있고 장중이면 `재시도`(확인창 → 새 행, 발주됨 옆 '재시도', 칩에 재시도 건수)(2026-09-09). 표의 '무인' 열은 실행 전에도 줄별로 `🤖 09:01 발주`(켜진 방향) / `수동`(꺼진 방향, 롤오버에 이유) / `수동(취소)` / `—` — 실행 전에는 같은 줄의 옛 행(전환 정리로 취소된 승인 행 등)을 무시하고 예고만 보인다(2026-09-08). 실행 후에는 실제 결과.
