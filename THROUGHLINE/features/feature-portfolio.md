...
- **수익률 추이 차트 (2026-09-08 지시 "109 는 다른 뜻처럼 보인다")**: 축·라벨·마지막 값을 TWR 지수(시작 100)가 아니라 **시작 대비 %**(+9.20%)로 표시, 0% 기준 점선. 마우스 롤오버 툴팁에 **날짜 · 수익률 · 그날 총 평가액**(`equity`). 데이터(`/portfolio/equity` 의 `index`·`equity`)는 그대로, 화면에서 −100.
