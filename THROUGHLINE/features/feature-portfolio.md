...
- **실시간 현재가 vs 주문선 (2026-09-09 지시)**: 주문표 아래 카드 — 워커의 10초 현재가 폴링이 Redis 시계열 `quotes:series:{code}:{date}`(≤3,000점, TTL 24h)에 누적하고, 화면은 `GET /quotes/series?code=` 로 그날 누적을 받은 뒤 WS `/ws/quotes` 구독으로 10초마다 한 점씩 이어 그린다(WS 실패 시 15초 폴링). 그리드 매수·초기 진입(빨강)·익절(파랑)·갭 기준(회색)을 점선 가격선으로, 옆에 줄별 **현재가 대비 거리 %**(가장 가까운 선 강조). 최근 60분 창이 오른쪽 끝을 따라 흐르고 '하루 전체 보기' 토글. 누적이 비어 있으면(장중 워커 재시작) KIS 1분봉으로 1회 백필(10분 잠금, 13호출). **KIS 호출 추가 없음** — 기존 폴링(7종목 × 6회/분 ≈ 0.7건/초) 재사용.
...
- **무인 상태 표시 (2026-09-08 상태 줄 → 2026-09-09 박스 제거 지시)**: 붉은 박스 대신 주문표 **제목 오른쪽 칩** 한 줄 — `🤖 09:01 발주 예정 · 매수 ON · 매도 OFF` / `✅ 발주 3건` / `⛔ 정지 — 사유 [다시 켜기]` / `⚠️ 09:01 기록 없음`(정지·기록 없음만 붉게, 상세·계좌·발주 n줄·수동 m줄은 롤오버). **'이번 실행일 무인 취소'는 '무인' 열 헤더의 `취소` 링크**(취소 뒤 09:00 전이면 `되돌리기`), 헤더 ⓘ 에 설명. 표의 '무인' 열은 실행 전에도 줄별로 `🤖 09:01 발주`(켜진 방향) / `수동`(꺼진 방향, 롤오버에 이유) / `수동(취소)` / `—` — 실행 전에는 같은 줄의 옛 행(전환 정리로 취소된 승인 행 등)을 무시하고 예고만 보인다(2026-09-08). 실행 후에는 실제 결과.
