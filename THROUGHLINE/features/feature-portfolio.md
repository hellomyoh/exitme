...
- 무인 실행 계좌별 스위치(2026-09-07, 0024): `GET /settings/auto-exec` → {default, accounts[{…, auto_exec}]}, `PUT /settings/auto-exec {buy, sell, preopen_cancel?}` = 기본값 + 모든 계좌 일괄, `PUT /settings/auto-exec/accounts/{aid} {buy?, sell?, preopen_cancel?}`. `GET /portfolio/{pid}/auto-exec` 의 `allowed` 는 연결 계좌 값, `account` 포함.
- 완전 무인(2026-09-07): `PUT /portfolio/{pid}/auto-exec/auto-approve {enabled, market_reserve, daily_buy_cap_pct}` (기본 켬; 켜면 응답에 `run_now` 즉시 실행 결과. 상한은 총자산 대비 %, 기본 20, 0 = 없음), `POST /portfolio/{pid}/auto-exec/auto-approve/run-now`, `POST /portfolio/{pid}/orders/cancel-all {stop, since}` → {cancelled, failed, filled_untouched, items, auto_exec}. `GET /portfolio/{pid}/auto-exec` 응답에 `auto_approve`·`auto_approve_last`.
