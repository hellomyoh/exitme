...
- **계획 vs 체결 대조 표시 (2026-09-08 지시 "참고용은 이모티콘 + 롤오버, 문장은 핵심만")**: `warn`(계획에 없던 거래·수량 불일치)만 ⚠️ 배너(한 줄 문구). `info`(지정가 미체결 = 대부분 정상)는 주문표 제목 옆 **ⓘ** 툴팁으로 — `reconcile.items[].label/plan/filled` 사용. 차이 없으면 표시 없음.
