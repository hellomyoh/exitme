...
- 보수: 일할(연 365일 기준) 평가액 차감 — 레버리지 0.64%, KODEX 200 0.15%. 벤치마크(매수보유)에도 KODEX 200 보수는 동일 적용. **뜻 확정 (2026-09-12, 감사 A4)**: ETF 시장가격은 TER 차감 후 NAV 를 따르므로 이 차감은 운용보수의 재계상이 아니라 **모델에 없는 비용(스프레드·부분체결·미체결 기회비용·현금 재투자 지연)의 보수적 대리치**이며 값만 TER 과 같게 둔다. 8.6년 누적 영향 약 +3.6%p(차감 0 대비). 벤치마크에 같은 값을 차감해 상대 비교는 불변이고 회귀 기준 KPI 가 이 규약 위에 있어 유지한다. 분배금은 모델하지 않는다(재투자 경로 없음 — 수익률이 그만큼 낮게 나오는 보수적 방향, 감사 A7). 실계좌 세후 성과 인증에는 쓰지 않는다.
...
- 워밍업 미달 시작일 차단, 시세 결측 구간 포함 실행 시 명시적 오류(2026-09-12 구현, 감사 A5: 두 종목 **공통 구간 안**에서 한쪽에만 있는 거래일은 409 로 거부하고 날짜를 알린다 — 구간 양끝의 상장·적재 시차는 종전처럼 교집합으로 맞춘다. 엔진 진입점도 길이가 아니라 날짜 동일성을 검사), 60s 초과 잡 타임아웃 처리.
