...
전략을 과거 데이터로 검증한다. 조건 설정 → 비동기 실행(진행률) → 결과의 3스텝 위저드로, RAVG v2 프리셋과 절제(ablation) 비교를 내장한다.

## 2. 범위

### 포함 범위

- Step 1 조건 설정: 자본금·기간·기준봉·유니버스·비용 + RAVG v2 프리셋(절제 플래그 5종) + 범용 조건식(지표 AND/OR)·프리셋 저장/복제
...
RAVG v2 프리셋 선택 → 2015~2025·자본 1억 설정 → 실행(진행률 확인) → KPI를 KODEX 200 매수보유와 비교 →
...
- RAVG v2 프리셋에 절제 플래그 5종(①상승장 익절 제거 ②하방 변동성 ③레짐 판정 단축 ④레버리지 모듈(Emax 1.30) ⑤갭 필터+잔여예산). 정본 §11의 검증 순서를 UI에 안내.
