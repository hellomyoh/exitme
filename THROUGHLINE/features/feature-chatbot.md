...
## 9. 권한별 프롬프트 분리 (2026-09-05 지시)

- 관리자: 전략 상세(STRATEGY_DETAIL — 수식·계수·임계값) 포함, 전 도구, 제한 없음.
- 일반: 개념 설명본(STRATEGY_PLAIN — 수치 없는 작동 원리) + **공개 제한 계약(RESTRICT_CONTRACT,
  코드 고정·본문/추가 지침으로 해제 불가)** — 수식·계수·임계값·파라미터 값 노출 금지, 유도 요청에도
  동일, 본인 계좌 수치(주문 가격/수량·보유·손익)는 본인 데이터라 표시 허용. algorithm_params 도구는
  관리자 전용(일반 세션 도구 목록에서 제외 + 서버 이중 차단).
- 검증: 프롬프트 역할 분기·도구 차단 테스트(비밀 문자열 5종 부재 확인) + 실 LLM 로 일반 계정이
  '정확한 공식·계수' 요청 시 거절+개념 설명 응답·수치 유출 0 확인. 전체 156 passed.

## 10. 운영 기능 지식·도구 보강 (2026-09-07 지시 "챗봇에서 관련 내용을 답변 가능한 상태인지 검토·보완")

- 검토 결과: 프롬프트는 "발주는 사용자가 HTS 에서 직접 한다"는 9/4 시점 문장이었고, 9/5~9/7 에 들어온 증권사 연동·예약주문·무인 실행·
  사전 갭 취소·완전 무인·예수금 대조·매매 로그·텔레그램 알림을 전혀 몰랐다. 도구도 이 상태를 읽을 수 없어 "왜 발주가 안 됐나/정지됐나" 에
  답할 수 없었다.
- 보완: ① `OPERATIONS_KNOWLEDGE`(관리자·일반 공통) — 기능별 화면 위치·동작·시각(08:57/09:01/15:45/16:45)·정지 조건·해제 방법.
  ② 전략 지식의 발주 문장을 세 경로(HTS 직접·예약주문·무인 실행)로 수정. ③ 코어 계약의 화면 안내 표를 확장(무인 승인·완전 무인·전량 취소 =
  실전매매, 허용 스위치·사전 갭 취소·알림 = 일반 설정, 기록 = 매매 로그). ④ 도구 2종 — `auto_exec_status`(설정 스위치, 포트별 정지·사유,
  마지막 무인 실행/자동 승인/사전 갭 확인, 완전 무인 설정, 예수금 대조, 살아 있는 주문 수, 알림 켬/준비 여부 — 채팅 ID·토큰은 내보내지 않음),
  `recent_logs`(GET /logs 병합 결과, level=warn 으로 실패만). ⑤ "오늘" 날짜를 KST 로(UTC 는 새벽에 하루 밀림).
- 검증: `tests/test_chat_ops.py` — 지식 키워드·도구 등록·표시명, 도구 응답 형태, 사용자 스코프(타인 포트 error).
