...
RAVG v2 매매 전략을 내장한 **백테스트 + 실전매매 기록 + 자산 대시보드** 통합 웹.
...
- **백테스트 3스텝 위저드** — 조건 설정 → 실행(진행률) → 결과(KPI·자산곡선·오버레이 비교), RAVG v2 프리셋 + 절제(ablation) 플래그
- **RAVG v2 전략 엔진** — 장 마감 후 일일 배치로 레짐·노출·다음 거래일 지정가 주문표 자동 생성
