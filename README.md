...
- **증권사 연동(KIS)** — 체결 자동 가져오기, 장 마감 동기화, 예수금 대조, 새 실전매매 시작 시 잔고 불러오기
- **무인 매매(ADR-009)** — 계좌별 매수/매도 플래그만 켜면 매 실행일 09:01 에 그 순간의 원장으로 주문표를 계산해 곧바로 발주·동결(시가 확인·갭 취소·원장 대조·하루 상한·매수가능 수량 축소·자동 정지), 09:15 감시·하트비트 헬스체크, 주문표 상태 한 줄과 '이번 실행일 무인 취소' 버튼
...
스크립트는 **자기 갱신**합니다: 체크아웃한 태그의 `scripts/deploy.sh` 가 실행 중인 것과 다르면 그것으로 다시 실행하므로 서버의 옛 스크립트로 시작해도 새 배포 절차가 돕니다. 자기 갱신이 없는 옛 스크립트(v0.11.0 이하)로 처음 올릴 때만 새 스크립트를 먼저 꺼내 실행하세요: `git fetch --tags origin && git show v0.11.1:scripts/deploy.sh > /tmp/deploy.sh && bash /tmp/deploy.sh v0.11.1`.
