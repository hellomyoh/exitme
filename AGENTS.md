...
## 버저닝과 태그 (2026-09-05 지시)

- 버전은 `a.b.c` 세 자리, git tag 는 `v` 접두 (예: `v1.2.3`). 태그는 **사용자 지시가 있을 때** main 의 머지 커밋에 붙이고 `git push origin <tag>` 로 올린다. 자의로 태그를 만들지 않는다.
- `a` 메이저: 앱의 큰 변화 — 런칭, UI 전면 개편 등 완전한 변화가 있을 때만.
- `b` 마이너: 규모가 큰 기능 추가 (예: 매매일지, 증권사 연동 같은 새 메뉴·모듈). 사용자 판정 예(2026-09-05): 매매일지 청산·계정 자산 초기화 같은 **새 동작이 추가**되면 마이너다.
- `c` 패치: 버그 수정, 문구·배치 조정, 스크립트 보강 등 기존 동작을 고치는 변경 — 새 동작이 없는 태그가 여기에 속한다.
- 태그 메시지에는 포함된 PR 번호와 한 줄 요약을 적는다(`git tag -a v1.0.1 -m "..."`). 어느 자리를 올릴지 애매하면 낮은 자리를 택하고 사용자에게 알린다.
- 태그를 붙일 커밋에는 `apps/api/app/VERSION` 을 같은 번호로 갱신한다 — `/health` 의 `version` 과 화면 왼쪽 아래 표시가 이 파일을 읽는다(빌드 시각·DB 리비전과 함께 배포 확인용).
- 시작점: `v0.1.0` = 2026-09-05 main(a9ec08f) — 실전매매·시뮬레이션·매매일지·증권사 연동(체결 가져오기·예약주문 접수). 정식 런칭 전이므로 `0.x` 로 시작하고, 런칭 시 `1.0.0` 으로 올린다.
...
- **오늘 기준 조회창을 타는 데이터에 고정 날짜를 심지 않는다** (2026-09-15, 시간 폭탄 2건 실측). 코드가 `today − N일`
  같은 상대 구간을 읽으면 고정 날짜 씨앗은 시간이 지나 창 밖으로 밀려 **나중에** 터진다. 씨앗과 기대값을 모두
  `today − k` 로 잡고, 코드와 **같은 시계**(KST/`date.today()`)를 쓴다. 공유 CI DB 에 넣은 가짜 봉은 같은 상대
  구간으로 반드시 지운다 — 남기면 다른 테스트의 '마지막 종가'를 오염시킨다.
